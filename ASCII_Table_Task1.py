...
class PayrollSystem:
    ...
    def run_payroll(self):
        ...
        w_id = 10
        w_pay = 14
        pembatas = f"+{'-' * (w_name + 2)}+{'-' * (w_id + 2)}+{'-' * (w_pay + 2)}+"
        
        print("\nPAYROLL REPORT")
        print(pembatas)
        print(f"| {'Employee Name':<{w_name}} | {'ID':<{w_id}} | {'Weekly Pay':>{w_pay}} |")
        print(pembatas)
        ...
        for emp in self._listEmployees:
            if isinstance(emp, SalariedEmployee):
                ...
                print(f"| {emp.name:<{w_name}} | {emp.emp_id:<{w_id}} | ${pay:>{w_pay-1}.2f} |")
                total_payroll += pay
        ...
        print(pembatas)
        w_label = w_name + w_id + 3 
        print(f"| {'TOTAL PAYROLL':<{w_label}} | ${total_payroll:>{w_pay-1}.2f} |")
        print(pembatas)
        print(" ")
